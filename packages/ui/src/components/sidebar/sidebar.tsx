...
import { getThemeVar } from '#components/theme-utils';
...
const SidebarProvider = ({
    ...
    const mobileBreakpoint = parseInt(getThemeVar('--mobile-width'));
        ...
        >
            ...
            <div
                className={cn(
                    ...
                    variant === 'floating' || variant === 'inset'
                        ? 'group-data-[collapsible=icon]:w-[calc((--sidebar-width-icon)_+_(--spacing-4))]'
            ...
            <div
                className={cn(
                    ...
                    variant === 'floating' || variant === 'inset'
                        ? 'p-2 group-data-[collapsible=icon]:w-[calc((--sidebar-width-icon)_+_(--spacing-4)_+2px)]'
...
const SidebarRail = ({ className, ref, ...props }: React.ComponentPropsWithRef<'button'>) => {
    ...
    return (
        <button
            ...
            className={cn(
                'absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear hover:after:bg-sidebar-border after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex',
...
const SidebarGroupLabel = <SlotType extends React.ElementType = 'div'>({
    className,
    asChild = false,
    ref,
    ...props
}: React.ComponentPropsWithRef<SlotType> & { asChild?: boolean }) => {
    ...
    return (
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground/70 ring-sidebar-ring [&>svg]:size-5 flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium outline-none transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:shrink-0',
...
const SidebarGroupAction = ({
    ...
    return (
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground [&>svg]:size-5 absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-none transition-transform focus-visible:ring-2 [&>svg]:shrink-0',
...
const SidebarMenu = ({ className, ref, ...props }: React.ComponentPropsWithRef<'ul'>) => (
    <ul ref={ref} data-sidebar="menu" className={cn('flex min-w-0 flex-col gap-2', className)} {...props} />
);
...
const SidebarMenuAction = ({
    ...
    return (
        <Comp
            ...
            className={cn(
                'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground peer-hover/menu-button:text-sidebar-accent-foreground [&>svg]:size-5 absolute right-1 top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-none transition-transform focus-visible:ring-2 [&>svg]:shrink-0',
                // Increases the hit area of the button on mobile.
                'after:absolute after:-inset-2 after:md:hidden',
...
const SidebarMenuSub = ({ className, ref, ...props }: React.ComponentPropsWithRef<'ul'>) => (
    <ul
        ...
        className={cn(
            'border-sidebar-border mx-4 flex min-w-0 translate-x-px flex-col gap-1 border-l px-3 py-0.5',
            'group-data-[collapsible=icon]:hidden',
            className
        )}
        {...props}
